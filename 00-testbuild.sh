...
# "make" produces the "ims" executable needed for cs154-2015 p5ims
#
...
function cleanup {
  ...
  # as generated by svn co
  rm -rf $tmpdir
}
# (using ~ instead of /tmp because /tmp may be mounted with noexec)
tmpdir=$(mktemp -d ~/cs154tmp.XXXXXXXX)
chmod 700 $tmpdir
